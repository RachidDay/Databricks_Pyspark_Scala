...
CrimeDataLosAngeles=spark.read.parquet('dbfs:/mnt/training/crime-data-2016/Crime-Data-Los-Angeles-2016.parquet').createOrReplaceTempView('CrimeDataLosAngeles')

# COMMAND ----------

# MAGIC %scala
# MAGIC val CrimeDataLosAngeles_Scala=spark.read.parquet("dbfs:/mnt/training/crime-data-2016/Crime-Data-Los-Angeles-2016.parquet")
# MAGIC   .createOrReplaceTempView("CrimeDataLosAngeles_Scala")
# MAGIC
...
CrimeDataPhiladelphia=spark.read.parquet('dbfs:/mnt/training/crime-data-2016/Crime-Data-Philadelphia-2016.parquet').createOrReplaceTempView('CrimeDataPhiladelphia')

# COMMAND ----------

# MAGIC %scala
# MAGIC val CrimeDataPhiladelphia_scala=spark.read.parquet("dbfs:/mnt/training/crime-data-2016/Crime-Data-Philadelphia-2016.parquet").createOrReplaceTempView("CrimeDataPhiladelphia_scala")
...
CrimeDataDallas=spark.read.parquet('dbfs:/mnt/training/crime-data-2016/Crime-Data-Dallas-2016.parquet').createOrReplaceTempView('CrimeDataDallas')

# COMMAND ----------

# MAGIC %scala
# MAGIC val CrimeDataDallas_scala=spark.read.parquet("dbfs:/mnt/training/crime-data-2016/Crime-Data-Dallas-2016.parquet").createOrReplaceTempView("CrimeDataDallas_scala")
...
CrimeDataLosAngeles=spark.table('CrimeDataLosAngeles').display()

# COMMAND ----------

obberyLosAngeles=spark.table("crimeDataLosAngeles").filter(F.lower(F.col("crimeCodeDescription")).like("robbery")).createOrReplaceTempView("RobberyLosAngeles")
...
# MAGIC %scala
# MAGIC val RobberyLosAngeles_scala=spark.table("crimeDataLosAngeles_scala").filter(lower(col("crimeCodeDescription")).like("robbery")).createOrReplaceTempView("RobberyLosAngeles_scala")
...
RobberyPhiladelphia=spark.table('CrimeDataPhiladelphia').filter((F.lower(F.col('ucr_general_description')).like('robbery')) ).createOrReplaceTempView('RobberyPhiladelphia')

# COMMAND ----------

# MAGIC %scala
# MAGIC val obberyPhiladelphia_scala =spark.table("CrimeDataPhiladelphia_scala").filter((lower(col("ucr_general_description")).like("robbery")) ).createOrReplaceTempView("RobberyPhiladelphia_scala")
...
CrimeDataDallas=spark.table('CrimeDataDallas').display()

# COMMAND ----------


RobberyDallas=spark.table('CrimeDataDallas').filter(F.lower(F.col('typeOfIncident')).like('robbery%')).createOrReplaceTempView('RobberyDallas')

# COMMAND ----------

# MAGIC %scala
# MAGIC val RobberyDallas_scala=spark.table("CrimeDataDallas_scala").filter(lower(col("typeOfIncident")).like("robbery%")).createOrReplaceTempView("RobberyDallas_scala")
...
obberiesByMonthLosAngeles=spark.table('RobberyLosAngeles').groupBy(F.month(F.col('timeOccurred')).alias('month')).agg(F.count("*").alias('robberies')).createOrReplaceTempView('RobberiesByMonthLosAngeles')

# COMMAND ----------

# MAGIC %scala
# MAGIC val RobberyLosAngeles_scala =spark.table("RobberyLosAngeles_scala").groupBy(month(col("timeOccurred")).alias("month")).agg(count("*").alias("robberies")).createOrReplaceTempView("RobberiesByMonthLosAngeles_scala")
...
RobberiesByMonthPhiladelphia=spark.table('RobberyPhiladelphia').groupBy(F.month(F.col('dispatch_date_time')).alias('month')).agg(F.count("*").alias('robberies')).createOrReplaceTempView('RobberiesByMonthPhiladelphia')

# COMMAND ----------

# MAGIC %scala
# MAGIC val RobberiesByMonthPhiladelphia_scala=spark.table("RobberyPhiladelphia_scala").groupBy(month(col("dispatch_date_time")).alias("month")).agg(count("*").alias("robberies")).createOrReplaceTempView("RobberiesByMonthPhiladelphia_scala")
...
RobberiesByMonthDallas=spark.table('RobberyDallas').groupBy(F.month(F.col('startingDateTime')).alias('month')).agg(F.count("*").alias('robberies')).createOrReplaceTempView('RobberiesByMonthDallas')

# COMMAND ----------

# MAGIC %scala
# MAGIC val RobberiesByMonthDallas_scala=spark.table("RobberyDallas_scala").groupBy(month(col("startingDateTime")).alias("month")).agg(count("*").alias("robberies")).createOrReplaceTempView("RobberiesByMonthDallas_scala")
...
RobberyLosAngeles=spark.table('RobberyLosAngeles').groupBy(F.month(F.col('timeOccurred')).alias('month')).agg(F.count("*").alias('robberies')).display()
...
RobberyPhiladelphia=spark.table('RobberyPhiladelphia').groupBy(F.month(F.col('dispatch_date_time')).alias('month')).agg(F.count("*").alias('robberies')).display()
...
RobberyDallas=spark.table('RobberyDallas').groupBy(F.month(F.col('startingDateTime')).alias('month')).agg(F.count("*").alias('robberies')).display()
...
RobberiesByMonthLosAngeles=spark.table('RobberiesByMonthLosAngeles').withColumn('city' , F.lit('Los Angeles')).sort(F.col('month').desc()).unionAll(spark.table('RobberiesByMonthPhiladelphia').withColumn('city' , F.lit('Philadelphia'))).sort(F.col('month').desc()).unionAll(spark.table('RobberiesByMonthDallas').withColumn('city' , F.lit('Dallas'))).sort(F.col('month').desc()).display()

# COMMAND ----------

RobberiesByMonthLosAngeles=spark.table('RobberiesByMonthLosAngeles').withColumn('city' , F.lit('Los Angeles')).unionAll(spark.table('RobberiesByMonthPhiladelphia').withColumn('city' , F.lit('Philadelphia'))).unionAll(spark.table('RobberiesByMonthDallas').withColumn('city' , F.lit('Dallas'))).sort(F.col('robberies').desc(),F.col('month').desc()).display()

# COMMAND ----------

CombinedRobberiesByMonth=spark.table('RobberiesByMonthLosAngeles').withColumn('city' , F.lit('Los Angeles')).unionAll(spark.table('RobberiesByMonthPhiladelphia').withColumn('city' , F.lit('Philadelphia'))).unionAll(spark.table('RobberiesByMonthDallas').withColumn('city' , F.lit('Dallas'))).sort(F.col('robberies').desc()).createOrReplaceTempView('CombinedRobberiesByMonth')

# COMMAND ----------

# MAGIC %scala
# MAGIC val CombinedRobberiesByMonth_scala=spark.table("RobberiesByMonthLosAngeles_scala").withColumn("city" , lit("Los Angeles")).unionAll(spark.table("RobberiesByMonthPhiladelphia_scala").withColumn("city", lit("Philadelphia"))).unionAll(spark.table("RobberiesByMonthDallas_scala").withColumn("city" , lit("Dallas"))).sort(col("robberies").desc).createOrReplaceTempView("CombinedRobberiesByMonth_scala")
...
city_data = spark.table('CityData').select('city', 'estPopulation2016').rdd.collectAsMap()
def rate(city, crime):
    if city in city_data:
        return crime / city_data[city]
    else:
        return None
rate_udf = F.udf(rate, FloatType())
df = spark.table('CombinedRobberiesByMonth')
df.withColumn('robberyRate', rate_udf(col('city'), col('robberies'))).createOrReplaceTempView('RobberyRatesByCity')
...
# MAGIC val cityData = spark.table("CityData").select("city", "estPopulation2016").collect().map(row => (row.getString(0), row.getLong(1))).toMap
# MAGIC def rate(city: String, crime: Int): Option[Float] = {
# MAGIC   cityData.get(city).map(population => crime.toFloat / population)
# MAGIC }
# MAGIC val rateUDF = udf(rate _).asNondeterministic()
# MAGIC val df = spark.table("CombinedRobberiesByMonth")
# MAGIC df.withColumn("robberyRate", rateUDF(col("city"), col("robberies"))).createOrReplaceTempView("RobberyRatesByCity_scala")  
...
# MAGIC # TEST - Run this cell to test your solution.
# MAGIC
# MAGIC results = [ (r.city, r.month, "{0:0.6f}".format(r.robberyRate)) for r in spark.sql("SELECT * FROM RobberyRatesByCity_scala").collect() ]
# MAGIC

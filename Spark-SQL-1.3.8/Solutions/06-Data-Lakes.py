...
# MAGIC -- ANSWER
# MAGIC
# MAGIC CREATE OR REPLACE TEMPORARY VIEW CrimeDataChicago
# MAGIC   USING parquet
# MAGIC   OPTIONS (
# MAGIC     path "dbfs:/mnt/training/crime-data-2016/Crime-Data-Chicago-2016.parquet"
# MAGIC   );
# MAGIC
# MAGIC SELECT * FROM CrimeDataChicago
...
# MAGIC %sql
# MAGIC -- ANSWER
# MAGIC -- Find the distinct list of offenses
# MAGIC
# MAGIC SELECT DISTINCT(primaryType)
# MAGIC FROM CrimeDataChicago
# MAGIC ORDER BY primaryType
...
# MAGIC -- ANSWER
# MAGIC
# MAGIC CREATE OR REPLACE TEMPORARY VIEW HomicidesChicago AS
# MAGIC   SELECT month(date) as month, primaryType as offense
# MAGIC   FROM CrimeDataChicago
# MAGIC   WHERE lower(primaryType) = 'homicide';
# MAGIC
# MAGIC SELECT * FROM HomicidesChicago
...
# MAGIC -- ANSWER
# MAGIC
# MAGIC CREATE OR REPLACE TEMPORARY VIEW AllHomicides AS
# MAGIC   SELECT * FROM HomicidesNewYork
# MAGIC     UNION ALL
# MAGIC   SELECT * FROM HomicidesBoston
# MAGIC     UNION ALL
# MAGIC   SELECT * FROM HomicidesChicago;
# MAGIC
# MAGIC SELECT * FROM AllHomicides
...
# MAGIC dbTest("SQL-L6-allHomicides-count", 1203, allHomicides)
# MAGIC
# MAGIC print("Tests passed!")
...
# MAGIC -- ANSWER
# MAGIC
# MAGIC CREATE OR REPLACE TEMPORARY VIEW HomicidesByMonth AS
# MAGIC   SELECT month, count(*) AS homicides
# MAGIC   FROM AllHomicides
# MAGIC   GROUP BY month
# MAGIC   ORDER BY month;
# MAGIC
# MAGIC SELECT * FROM HomicidesByMonth
...
# MAGIC
# MAGIC dbTest("SQL-L6-allHomicides-0", 83, allHomicides[0].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-1", 68, allHomicides[1].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-2", 72, allHomicides[2].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-3", 76, allHomicides[3].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-4", 105, allHomicides[4].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-5", 120, allHomicides[5].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-6", 116, allHomicides[6].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-7", 144, allHomicides[7].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-8", 109, allHomicides[8].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-9", 109, allHomicides[9].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-10", 111, allHomicides[10].homicides)
# MAGIC dbTest("SQL-L6-allHomicides-11", 90, allHomicides[11].homicides)
# MAGIC
# MAGIC print("Tests passed!")
